...
const fetchUserType = async (userID) => {
  try {
    const response = await fetch(`https://us-central1-ectable-7april.cloudfunctions.net/ectable/api/User/${userID}`);
...
const fetchRestId = async(query) =>{
    ...
    const api = `https://us-central1-ectable-7april.cloudfunctions.net/ectable/api/restaurants/chef`;
...
export const AuthProvider = ({children}) =>{
  ...
  if(isLoggedIn){
    ...
    try {
      const response = await axios.post("https://us-central1-ectable-7april.cloudfunctions.net/ectable/api/diner/login", {
        email: email,
        password: password,
      });
