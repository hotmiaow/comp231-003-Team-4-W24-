...
import { readMenuByID } from "./api-menu";
import {
  Table,
  TableBody,
  TableCell,
...
function BookingManagement() {
        ...
        try {
          ...
          const emailInfos = await Promise.all(data.map(async (item) => {
            ...
            try {
              ...
              let menuName = "No menu selected"; // Default text in case there's no menu or error
              console.log("Menu selection:", item.menuSelection);

              if (item.menuSelection && item.menuSelection.length > 0) {
                menuName = item.menuSelection.join(", ");
              }

              return {
                ...item,
                dinerEmail: emailInfo.email,
                menuSelectionNames: menuName,
              };

            } catch (error) {
              console.error(`Error processing reservation item ${item.id}:`, error);
              return { ...item, error: "Failed to process this item" };
            }
          }));

          setReservations(emailInfos);
        } catch (e) {
          console.error(`Error fetching reservation emails: ${e}`);
        }
      };
  ...
  //   try {
  //     const emailInfos = await Promise.all(
  //       data.map(async (item) => {
  //         if (item.dinerId) {
  //           const emailInfo = await readEmail(item);
  //           let menuName = "No menu selected"; // Default text in case there's no menu or error
  //           console.log(item.menuSelection)
  //             try{
  //                 if (item.menuSelection && item.menuSelection.length > 0) {
  //                   menuName = item.menuSelection.join(", ");
  //                 }
  ...
  //             } catch (error) {
  //                 console.error(
  //                   `Error fetching menus for ID ${menus}:`,
  //                   error
  //                 );
  //                 return "Error retrieving menu";
  //               }
  //               try{
  //                 return {
  //                   ...item,
  ...
  //             // const menuPromises = item.menuSelection.map(async (menus) => {
  //             //   try {
  //             //     console.log(menus);
  //             //     // const menuDataArray = await readMenuByID(menuId);
  //             //     if(menus && menus.length>0){
  //             //       const itemArray = menus.join(", ");
  //             //       return itemArray;
  //             //     }else {
  ...
  //                 // if (menuDataArray && menuDataArray.length > 0) {
  //                 //   // Process each menu item in the array
  //                 //   const menuNames = menuDataArray
  //                 //     .map((menuData) => menuData.name)
  //                 //     .join(", ");
  //                 //   return menuNames; // Returns a string of menu names separated by commas
  //                 // } 
                
            
  //             // const menuNames = await Promise.all(menuPromises);
              ...
              {reservations &&
                reservations.map((reservation, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {reservation.menuSelectionNames
                        ? reservation.menuSelectionNames
                        : "No menu selected"}
                    </TableCell>{" "}
                    {/* Display the menu names here */}
                    <TableCell>
